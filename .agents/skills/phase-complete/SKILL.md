---
name: phase-complete
description: Complete the active delivery phase only when its backlog is empty. Use when the user asks to close a phase by moving its design doc to `plans/done/`, deleting the active backlog file, and updating `plans/PHASES.md`.
---
...
3. Validate that the phase backlog is actually empty before any edits:
...
4. Check destination paths under `plans/done/`.
