...
- [ ] Tests include a fake or minimal non-OpenAI harness path that exercises startup wiring and send flow without depending on OpenAI-specific construction.
...
- [ ] Step 2: Introduce a fake or minimal harness fixture/helper that proves the app can boot and execute the normal send flow without OpenAI-specific startup wiring. — files: `tests/test_main_routes.py`, `tests/conftest.py`, `tests/test_chat_turn_service.py`, `tests/test_harness_registry.py`
...
- [ ] Add a regression test for readiness or diagnostics metadata under non-default harness wiring so observability stays normalized across harness implementations -> covers AC: Tests include a fake or minimal non-OpenAI harness path that exercises startup wiring and send flow without depending on OpenAI-specific construction.
