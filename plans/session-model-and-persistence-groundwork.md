...
## Acceptance Criteria
- [x] Existing user-visible flow is preserved: `/`, `/chat-start`, `/chats/{chat_id}`, and `/send-message-htmx` continue to behave as today, including hidden `chat_session_id` form wiring.
...
- [x] Regression tests cover schema migration/backfill, run-link persistence, duplicate replay behavior, and no-regression route send/restore paths.
