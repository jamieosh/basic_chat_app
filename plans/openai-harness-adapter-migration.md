...
- [ ] The main send path depends on normalized `ChatHarness.run()` execution and normalized failure codes rather than legacy `process_message()` monkeypatching or OpenAI SDK exception assumptions.
- [ ] OpenAI-specific error translation is localized to `agents/openai_agent.py` and app/service failure presentation only consumes normalized harness failure codes.
- [ ] Legacy compatibility aliases that keep provider-shaped failure codes alive in app-facing behavior are removed or clearly reduced to non-routing compatibility shims.
- [ ] Regression coverage proves default OpenAI parity while locking route and service behavior to normalized harness results/failures.
...
- [ ] Step 2: Remove route/service reliance on provider-shaped or legacy compatibility failure handling, keeping only normalized harness failure presentation in the app layer. — files: `main.py`, `services/chat_turns.py`
- [ ] Step 3: Update unit and route tests to exercise the normalized harness path directly while preserving current OpenAI-backed behavior and prompt assembly parity. — files: `tests/test_openai_agent.py`, `tests/test_main_routes.py`, `tests/test_chat_harness_contract.py`, `tests/test_chat_turn_service.py`
